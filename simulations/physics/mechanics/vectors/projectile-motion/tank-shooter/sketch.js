...
function draw() {
  ...
  if (isNaN(firingVelocity)) { // For invalid input
  ...
  if (gameOver === "yes"){ // When Game Over
    textSize(80)
    fill(245, 0, 0)
    stroke(245, 0, 0)
  ...
  if (target == null && gameOver === "no"){
    ...
    if (ppButton.status == 1){target.update()} // Move target
    target.show();
  }
  ...
  if (target != null && projectile != null && gameOver === "no"){ // If target and projectile objects exist
    
    if( (dist(projectile.pos.x, projectile.pos.y, target.x, target.y) < (projectile.r + target.w/2 + 80/conversionFactor)) && projectile.pos.y > target.y){  // If projectile hits target or lands near target
  ...
  if (projectile != null && gameOver === "no"){ // If projectile object is present
    projectile.show()
    
    if (ppButton.status == 1){projectile.update()} // If unpaused, move projectile
  ...
  for (let i = 0; i < destroyedTargets.length; i++){destroyedTargets[i].show()} // Display all destroyed tank objects stored in the list 
...
function mouseClicked() {
  ...
  if (mouseX >= 10 && mouseX <= 82 && mouseY >= (height/2 - 10 - 20) && mouseY <= (height/2 -10 + 20)) { // If mouse clicked over "Fire" button
  ...
  if(mouseX >= 48 && mouseX <= 95 && mouseY >= height/2 +45  && mouseY <= height/2 + 70){  //To change velocity
